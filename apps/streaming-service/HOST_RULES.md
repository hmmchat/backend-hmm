...
**Current Behavior**: Room auto-ends (cannot exist with only 1 person)
**Status**: ✅ Handled
...
**Current**: Room continues until participant count drops to 1
**Status**: ✅ Handled - Room continues with PARTICIPANTs (Option B confirmed)

### ✅ Edge Case B: Room Ends While Broadcasting
**Scenario**: Room ends (1 person remains) while broadcast is active
...
**Current**: PARTICIPANT is removed, then room auto-ends (only 1 person remains)
...
| Room auto-ends (1 person) | ✅ | Handled in `removeParticipant()` |
...
### Rule: Room ends when 1 person remains
- **Room ends when participant count drops to 1** (irrespective of HOST or PARTICIPANT)
- **Example Flow**:
  ...
  5. c leaves → Room: d(PARTICIPANT) → **Room ends** (only 1 person)
...
✅ **Room ending behavior confirmed**: Room ends only when 1 person remains (Option B)
✅ **Broadcasting auto-stops**: When room ends, broadcasting stops and viewers are removed

### Confirmed Behavior:
- Room continues even if all HOSTS leave (as long as 2+ participants)
- Room ends only when 1 participant remains (irrespective of HOST/PARTICIPANT)
