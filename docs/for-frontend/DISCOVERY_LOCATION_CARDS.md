...
Countdown default **10s** (`CITY_HANDOFF_COUNTDOWN_SECONDS`). Empty/boxes poll default **2s** (`DISCOVERY_AVAILABLE_CITIES_POLL_MS`). Both also exposed via `GET /v1/discovery/ui-config` and embedded on `available-cities`.
...
| `DISCOVERY_AVAILABLE_CITIES_POLL_MS` | `2000` | Poll on city boxes / empty orbit |
