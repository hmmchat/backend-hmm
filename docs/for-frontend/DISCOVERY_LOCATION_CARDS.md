...
Countdown default **10s** (`CITY_HANDOFF_COUNTDOWN_SECONDS`). Empty/boxes poll default **5s** (`DISCOVERY_AVAILABLE_CITIES_POLL_MS`). Matchmaking Raincheck unlock default **3s** (`RAINCHECK_UNLOCK_SECONDS`). Meet rn waiting cancel (X) unlock default **10s** (`MEET_RN_CANCEL_UNLOCK_SECONDS`). All exposed via `GET /v1/discovery/ui-config` and embedded on `available-cities`.
...
| GET | `/v1/discovery/available-cities` | Query: `sessionId`, `limit` (1–50, default 3), `soloOnly`, optional `excludeCity`. Returns `{ cities: [{ city, label, intent, faceCardImageUrl, availableCount }], ui: { cityHandoffCountdownSeconds, availableCitiesPollMs, raincheckUnlockSeconds, meetRnCancelUnlockSeconds } }`. Cities are showable-for-you, intent required, ordered by count desc. |
| GET | `/v1/discovery/ui-config` | `{ ok, cityHandoffCountdownSeconds, availableCitiesPollMs, raincheckUnlockSeconds, meetRnCancelUnlockSeconds }` from env. |
...
| `MEET_RN_CANCEL_UNLOCK_SECONDS` | `10` | Meet rn waiting cancel (X) appears after this many seconds. `0` = immediate |
