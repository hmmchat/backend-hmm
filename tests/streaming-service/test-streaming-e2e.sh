...
WALLET_SERVICE_URL="${WALLET_SERVICE_URL:-http://localhost:3006}"
export WALLET_SERVICE_URL
...
if [ ! -z "$PULL_STRANGER_ROOM_ID" ] && [ "$PULL_STRANGER_ROOM_ID" != "null" ]; then
    ...
    if [ "$ENABLE_SUCCESS" = "true" ]; then
        # Verify room info shows pull stranger enabled (we'll check via status updates in next test)
...
SINGLE_JOIN_PART_ID="test-single-join-part-$TIMESTAMP"
...
if [ ! -z "$PULL_STRANGER_ROOM_ID" ] && [ "$PULL_STRANGER_ROOM_ID" != "null" ]; then
    ...
    OUTSIDER_EXISTS=$(echo "$GET_OUTSIDER_RESPONSE" | jq -r '.exists // true' 2>/dev/null)
    
    if [ "$OUTSIDER_EXISTS" = "false" ]; then
        test_result 0 "Get room for user not in pull stranger mode returns exists: false"
    else
        test_result 1 "Get room for outsider user should return exists: false"
    fi
else
    test_result 1 "Skipped: Previous test failed"
fi
